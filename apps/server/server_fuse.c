 ...
 */
/*
 * Module: FUSE 3 client-drive presentation for desktop servers.
...
#define SERVER_FUSE_MAX_NAME_BYTES 255u
#define SERVER_FUSE_MAX_PATH_BYTES 4096u
...
#define SERVER_FUSE_FILE_NON_DIRECTORY 0x00000040u
#define SERVER_FUSE_FILE_ATTRIBUTE_NORMAL 0x00000080u
...
typedef enum server_fuse_node_kind
{
    ...
    SERVER_FUSE_NODE_PEER = 2,
    SERVER_FUSE_NODE_VOLUME = 3,
    SERVER_FUSE_NODE_REMOTE = 4,
    SERVER_FUSE_NODE_CLIPBOARD_DIRECTORY = 5,
    SERVER_FUSE_NODE_CLIPBOARD_FILE = 6
} server_fuse_node_kind;

typedef enum server_fuse_pending_kind
{
    SERVER_FUSE_PENDING_LOOKUP = 1,
    SERVER_FUSE_PENDING_OPEN = 2,
    SERVER_FUSE_PENDING_OPENDIR = 3,
    SERVER_FUSE_PENDING_READ = 4,
...
{
    SERVER_FUSE_STAGE_OPEN = 1,
    SERVER_FUSE_STAGE_QUERY = 2,
    SERVER_FUSE_STAGE_CLOSE = 3,
    SERVER_FUSE_STAGE_IO = 4
} server_fuse_pending_stage;

typedef struct server_fuse_node
{
    fuse_ino_t inode;
    fuse_ino_t parent;
    uint64_t volume_id;
...
typedef struct server_fuse_pending
{
    uint64_t request_id;
    fuse_req_t request;
    fuse_ino_t inode;
    ...
    uint32_t stream_id;
    int terminal_error;
    char name[SERVER_FUSE_MAX_NAME_BYTES + 1u];
    server_fuse_pending_kind kind;
    server_fuse_pending_stage stage;
    int occupied;
} server_fuse_pending;

typedef struct server_fuse_clipboard_entry
{
    librdp_clipboard_file_metadata metadata;
    char name[SERVER_FUSE_MAX_NAME_BYTES + 1u];
} server_fuse_clipboard_entry;
...
struct server_fuse
{
    ...
    server_fuse_node* nodes;
    server_fuse_handle* handles;
...
static char* server_fuse_copy_string(const char* value)
{
...
static int server_fuse_name_valid(const char* name)
{
    size_t length = 0u;

    if (!name || name[0] == '\0')
        return 0;
    while (length <= SERVER_FUSE_MAX_NAME_BYTES && name[length] != '\0')
    {
        if (name[length] == '/' || name[length] == '\\')
            return 0;
        length++;
    }
    return length > 0u && length <= SERVER_FUSE_MAX_NAME_BYTES &&
           strcmp(name, ".") != 0 && strcmp(name, "..") != 0;
}

static int server_fuse_mount_path_secure(const char* path)
{
    struct stat status;
    DIR* directory = NULL;
...
static int server_fuse_config_valid(const server_fuse_config* config)
{
    return config && config->version == SERVER_FUSE_CONFIG_VERSION &&
           config->size >= sizeof(*config) && config->mount_path &&
           config->mount_path[0] != '\0' &&
           strlen(config->mount_path) <= SERVER_FUSE_MAX_PATH_BYTES &&
           config->max_nodes >= 4u && config->max_handles > 0u &&
           config->max_pending > 0u && config->max_directory_entries > 0u &&
...
static server_fuse_node* server_fuse_find_child(const server_fuse* provider,
                                                fuse_ino_t parent,
                                                const char* name)
{
    uint32_t index = 0u;
                                                 ...
                                                 uint64_t volume_id)
{
    ...
    for (index = 0u; index < provider->config.max_nodes; index++)
    {
        ...
        if (node->valid && node->kind == SERVER_FUSE_NODE_VOLUME &&
...
static server_fuse_node* server_fuse_volume_for_node(
    const server_fuse* provider, const server_fuse_node* node)
{
    if (!node)
        return NULL;
...
static server_fuse_pending* server_fuse_find_pending(
    const server_fuse* provider, uint64_t request_id)
{
    uint32_t index = 0u;
...
static server_fuse_pending* server_fuse_allocate_pending(
    server_fuse* provider, fuse_req_t request, server_fuse_pending_kind kind)
{
    uint32_t index = 0u;
...
static void server_fuse_clear_pending(server_fuse_pending* pending)
{
    if (pending)
        memset(pending, 0, sizeof(*pending));
}

static server_fuse_handle* server_fuse_allocate_handle(server_fuse* provider,
                                                       fuse_ino_t inode,
                                                       int directory, int local)
{
    uint32_t index = 0u;
...
static void server_fuse_clear_handle(server_fuse_handle* handle)
{
    if (!handle)
        return;
...
static int server_fuse_join_path(const char* parent, const char* name,
                                 char output[SERVER_FUSE_MAX_PATH_BYTES + 1u])
{
    size_t parent_len = parent ? strlen(parent) : 0u;
    size_t name_len = name ? strlen(name) : 0u;
    size_t separator = parent_len > 0u ? 1u : 0u;

    if (!name || !server_fuse_name_valid(name) ||
        parent_len > SERVER_FUSE_MAX_PATH_BYTES ||
        name_len > SERVER_FUSE_MAX_PATH_BYTES - parent_len - separator)
        return 0;
    if (parent_len > 0u)
...
static server_fuse_node* server_fuse_allocate_node(server_fuse* provider,
                                                   server_fuse_node_kind kind,
                                                   fuse_ino_t parent,
                                                   const char* name,
                                                   const char* path)
{
    ...
    if (!provider || kind < SERVER_FUSE_NODE_PEER ||
        kind > SERVER_FUSE_NODE_CLIPBOARD_FILE ||
        !server_fuse_name_valid(name) || !path ||
        strlen(path) > SERVER_FUSE_MAX_PATH_BYTES)
        return NULL;
    ...
    if (!node)
        return NULL;
    node->name = server_fuse_copy_string(name);
    node->path = server_fuse_copy_string(path);
...
static void server_fuse_invalidate_node(server_fuse* provider,
                                        server_fuse_node* node)
{
    if (!provider || !node || !node->valid ||
        node->kind == SERVER_FUSE_NODE_ROOT)
        return;
    if (provider->mounted)
...
static uint64_t server_fuse_filetime_seconds(uint64_t value)
{
    if (value <= SERVER_FUSE_UNIX_EPOCH_FILETIME)
        return 0u;
    return (value - SERVER_FUSE_UNIX_EPOCH_FILETIME) /
                                  ...
                                  struct stat* status)
{
    ...
    if (node->metadata_valid)
    {
        ...
        seconds = server_fuse_filetime_seconds(node->metadata.access_time);
        status->st_atime =
            seconds > (uint64_t)INT64_MAX ? (time_t)INT64_MAX : (time_t)seconds;
        seconds = server_fuse_filetime_seconds(node->metadata.write_time);
        status->st_mtime =
            seconds > (uint64_t)INT64_MAX ? (time_t)INT64_MAX : (time_t)seconds;
        seconds = server_fuse_filetime_seconds(node->metadata.change_time);
        status->st_ctime =
            seconds > (uint64_t)INT64_MAX ? (time_t)INT64_MAX : (time_t)seconds;
    }
}

static int server_fuse_node_is_directory(const server_fuse_node* node)
{
    if (!node)
        return 0;
    if (node->kind == SERVER_FUSE_NODE_REMOTE ||
...
static int server_fuse_status_errno(librdp_status status)
{
    switch (status)
    {
...
static int server_fuse_io_errno(uint32_t io_status)
{
...
static int server_fuse_completion_errno(
    const server_platform_drive_completion* completion)
{
    if (!completion)
        return EIO;
    if (completion->status != LIBRDP_STATUS_OK)
        return server_fuse_status_errno(completion->status);
    return server_fuse_io_errno(completion->io_status);
}

static uint64_t server_fuse_next_request_id(server_fuse* provider)
{
...
static int server_fuse_prepare_request(server_fuse* provider,
                                       server_fuse_pending* pending,
                                       const server_fuse_node* node,
                                       ...
                                       librdp_server_drive_operation operation)
{
    ...
    request->request_id = server_fuse_next_request_id(provider);
                                     ...
                                     uint32_t desired_access)
{
    ...
    request.operation.create_options =
        directory ? SERVER_FUSE_FILE_DIRECTORY : SERVER_FUSE_FILE_NON_DIRECTORY;
    pending->stage = SERVER_FUSE_STAGE_OPEN;
    provider->sink.request(&request, provider->sink.user_data);
    return 1;
}

static int server_fuse_submit_query_directory(
    ...
    const char* pattern, int initial)
{
    ...
    pending->stage = SERVER_FUSE_STAGE_QUERY;
    provider->sink.request(&request, provider->sink.user_data);
    return 1;
}

static int server_fuse_submit_read(server_fuse* provider,
                                   server_fuse_pending* pending,
                                   const server_fuse_node* node,
                                    ...
                                    librdp_server_drive_file_handle file)
{
    ...
    if (!server_fuse_prepare_request(provider, pending, node, &request,
        ...
        return 0;
    request.operation.file = file;
    pending->stage = SERVER_FUSE_STAGE_CLOSE;
    provider->sink.request(&request, provider->sink.user_data);
    return 1;
}
...
static int server_fuse_clipboard_node_matches(const server_fuse_node* node,
                                              uint32_t peer_id,
                                              uint32_t generation,
                                              uint64_t ownership_generation)
{
    return node &&
           (node->kind == SERVER_FUSE_NODE_CLIPBOARD_DIRECTORY ||
            node->kind == SERVER_FUSE_NODE_CLIPBOARD_FILE) &&
 ...
 */
static void server_fuse_clipboard_clear_internal(server_fuse* provider,
                                                 uint32_t peer_id,
                                                 uint32_t generation,
                                                 uint64_t ownership_generation)
{
    ...
    for (index = 0u; index < provider->config.max_pending; index++)
    {
        ...
        if (!pending->occupied ||
            pending->kind != SERVER_FUSE_PENDING_CLIPBOARD_READ)
            continue;
        node = server_fuse_find_node(provider, pending->inode);
        if (!server_fuse_clipboard_node_matches(node, peer_id, generation,
                                                ownership_generation))
            continue;
        if (provider->clipboard_cancel)
        {
            (void)provider->clipboard_cancel(
                peer_id, generation, ownership_generation, pending->request_id,
                provider->clipboard_user_data);
        }
        if (pending->occupied)
            server_fuse_reply_pending_error(pending, ESTALE);
    }
    for (index = 0u; index < provider->config.max_handles; index++)
    {
        server_fuse_handle* handle = &provider->handles[index];
        server_fuse_node* node =
            handle->occupied ? server_fuse_find_node(provider, handle->inode)
                             : NULL;
    ...
    for (index = 0u; index < provider->config.max_nodes; index++)
    {
        ...
        if (node->kind == SERVER_FUSE_NODE_CLIPBOARD_FILE &&
            server_fuse_clipboard_node_matches(node, peer_id, generation,
                                               ownership_generation))
            server_fuse_invalidate_node(provider, node);
    }
    for (index = 0u; index < provider->config.max_nodes; index++)
    {
        ...
        if (node->kind == SERVER_FUSE_NODE_CLIPBOARD_DIRECTORY &&
            server_fuse_clipboard_node_matches(node, peer_id, generation,
                                               ownership_generation))
            server_fuse_invalidate_node(provider, node);
    }
    ...
    if (provider->mounted)
    {
        (void)fuse_lowlevel_notify_inval_inode(provider->session, FUSE_ROOT_ID,
                                               0, 0);
    }
}

static int server_fuse_uri_byte_safe(uint8_t value)
{
...
static librdp_status server_fuse_append_file_uri(uint8_t** output,
                                                 size_t* length,
                                                 const char* path)
{
    ...
    {
        size_t bytes =
            server_fuse_uri_byte_safe((uint8_t)path[index]) ? 1u : 3u;

        if (encoded_len > SIZE_MAX - bytes)
            return LIBRDP_STATUS_LIMIT_EXCEEDED;
        encoded_len += bytes;
    }
    if (*length > SERVER_FUSE_MAX_CLIPBOARD_URI_BYTES ||
        overhead > SERVER_FUSE_MAX_CLIPBOARD_URI_BYTES - *length ||
        encoded_len > SERVER_FUSE_MAX_CLIPBOARD_URI_BYTES - *length - overhead)
        return LIBRDP_STATUS_LIMIT_EXCEEDED;
    ...
    *output = resized;
    ...
    for (index = 0u; index < path_len; index++)
    {
        ...
        if (server_fuse_uri_byte_safe(value))
...
/*
 * Decode the complete remote descriptor group before changing namespace
 * state. Node allocation, URI construction and ownership publication commit
 * together; any malformed descriptor or quota failure leaves the previous
 * clipboard generation intact.
 */
static librdp_status server_fuse_clipboard_publish_internal(
    ...
    int require_started)
{
    server_fuse_node* directory = NULL;
    server_fuse_clipboard_entry* entries = NULL;
    uint8_t* output = NULL;
    size_t output_len = 0u;
    uint32_t count = 0u;
    uint32_t index = 0u;
    char directory_name[SERVER_FUSE_MAX_NAME_BYTES + 1u];
    int directory_name_len = 0;
    librdp_status status = LIBRDP_STATUS_OK;

    if (!provider || peer_id == 0u || generation == 0u ||
        ownership_generation == 0u || !descriptors || descriptors_len == 0u ||
        !uri_list || !uri_list_len ||
        (require_started && (!provider->started || !provider->mounted ||
                             !provider->clipboard_request)))
        return LIBRDP_STATUS_INVALID_ARGUMENT;
    *uri_list = NULL;
    *uri_list_len = 0u;
    status =
        librdp_clipboard_file_group_count(descriptors, descriptors_len, &count);
    ...
    if (count == 0u || count > (uint32_t)INT32_MAX ||
        ...
        return LIBRDP_STATUS_LIMIT_EXCEEDED;
    entries = (server_fuse_clipboard_entry*)calloc(count, sizeof(*entries));
    if (!entries)
        return LIBRDP_STATUS_NO_MEMORY;
    for (index = 0u; index < count; index++)
    {
        ...
        status = librdp_clipboard_file_metadata_init(&entries[index].metadata);
        if (status == LIBRDP_STATUS_OK)
        {
            status = librdp_clipboard_file_group_get(
                descriptors, descriptors_len, index, &entries[index].metadata,
                entries[index].name, sizeof(entries[index].name), &name_len);
        }
        if (status != LIBRDP_STATUS_OK)
            break;
        if (name_len == 0u ||
            ...
            !server_fuse_name_valid(entries[index].name))
        {
    ...
    {
        server_fuse_clipboard_clear_internal(
            provider, provider->clipboard_peer_id,
            provider->clipboard_peer_generation,
            provider->clipboard_generation);
    }
    directory_name_len =
        snprintf(directory_name, sizeof(directory_name), "clipboard-%u-%u-%llu",
                 peer_id, generation, (unsigned long long)ownership_generation);
    if (directory_name_len <= 0 ||
        (size_t)directory_name_len >= sizeof(directory_name))
        return LIBRDP_STATUS_LIMIT_EXCEEDED;
    directory = server_fuse_allocate_node(
        provider, SERVER_FUSE_NODE_CLIPBOARD_DIRECTORY, FUSE_ROOT_ID,
    ...
    for (index = 0u; index < count; index++)
    {
        const server_fuse_clipboard_entry* entry = &entries[index];
        server_fuse_node* node = NULL;
        char visible_name[SERVER_FUSE_MAX_NAME_BYTES + 1u];
        char relative_path[SERVER_FUSE_MAX_PATH_BYTES + 1u];
        char absolute_path[SERVER_FUSE_MAX_PATH_BYTES + 1u];
        int absolute_len = 0;

        memcpy(visible_name, entry->name, strlen(entry->name) + 1u);
        if (server_fuse_find_child(provider, directory->inode, visible_name))
        {
            ...
            do
            {
                visible_len = snprintf(visible_name, sizeof(visible_name),
                                       "file-%u", suffix++);
                if (visible_len <= 0 ||
                    (size_t)visible_len >= sizeof(visible_name) || suffix == 0u)
                {
                    status = LIBRDP_STATUS_LIMIT_EXCEEDED;
                    break;
                }
            } while (server_fuse_find_child(provider, directory->inode,
                                            visible_name));
            if (status != LIBRDP_STATUS_OK)
                break;
        }
        if (!server_fuse_join_path(directory_name, visible_name, relative_path))
        {
            status = LIBRDP_STATUS_LIMIT_EXCEEDED;
            break;
        }
        node = server_fuse_allocate_node(
            provider, SERVER_FUSE_NODE_CLIPBOARD_FILE, directory->inode,
            visible_name, relative_path);
        if (!node)
        {
            status = LIBRDP_STATUS_LIMIT_EXCEEDED;
            break;
        ...
        node->metadata.attributes = LIBRDP_SERVER_DRIVE_FILE_ATTRIBUTE_READONLY;
        node->metadata.directory = 0u;
        node->metadata_valid = 1;
        absolute_len = snprintf(absolute_path, sizeof(absolute_path), "%s/%s",
                                provider->mount_path, relative_path);
        if (absolute_len <= 0 || (size_t)absolute_len >= sizeof(absolute_path))
        {
            status = LIBRDP_STATUS_LIMIT_EXCEEDED;
            break;
        }
        status =
            server_fuse_append_file_uri(&output, &output_len, absolute_path);
        if (status != LIBRDP_STATUS_OK)
            break;
    }
    if (status != LIBRDP_STATUS_OK)
    {
        free(entries);
        free(output);
        server_fuse_clipboard_clear_internal(provider, peer_id, generation,
                                             ownership_generation);
        return status;
    }
    free(entries);
...
static server_fuse_node* server_fuse_add_remote_node(
    server_fuse* provider, server_fuse_node* parent, const char* name,
    const librdp_server_drive_metadata* metadata)
{
    server_fuse_node* node =
        server_fuse_find_child(provider, parent->inode, name);
    char path[SERVER_FUSE_MAX_PATH_BYTES + 1u];
...
static int server_fuse_directory_name_matches(const char* expected,
                                              const char* actual)
{
    size_t index = 0u;
...
static int server_fuse_decode_lookup(
    server_fuse* provider, server_fuse_pending* pending,
    const server_platform_drive_completion* completion)
{
    server_fuse_node* parent = server_fuse_find_node(provider, pending->parent);
    librdp_server_drive_metadata metadata;
    char remote_name[SERVER_FUSE_MAX_NAME_BYTES + 1u];
    ...
    if (!parent ||
        ...
        name_length == 0u ||
        !server_fuse_directory_name_matches(pending->name, remote_name))
        return EPROTO;
    return server_fuse_add_remote_node(provider, parent, pending->name,
                                       &metadata)
               ? 0
               : ENOSPC;
}

static int server_fuse_handle_add_directory_entry(server_fuse* provider,
                                                  server_fuse_handle* handle,
                                                  fuse_ino_t inode)
{
...
static int server_fuse_decode_directory_page(
    server_fuse* provider, server_fuse_pending* pending,
    server_fuse_handle* handle,
    const server_platform_drive_completion* completion, size_t* decoded_count)
{
    server_fuse_node* parent = server_fuse_find_node(provider, handle->inode);
    size_t offset = 0u;
...
static void server_fuse_reply_directory(server_fuse* provider,
                                        server_fuse_pending* pending,
                                        server_fuse_handle* handle)
{
    server_fuse_node* directory =
        server_fuse_find_node(provider, handle->inode);
    ...
    if (capacity > SERVER_FUSE_MAX_REPLY_BYTES)
        capacity = SERVER_FUSE_MAX_REPLY_BYTES;
    buffer = capacity > 0u ? (char*)malloc(capacity) : NULL;
    if (capacity > 0u && !buffer)
    {
        server_fuse_reply_pending_error(pending, ENOMEM);
        return;
    }
    while (position < total)
    {
        ...
        {
            name = "..";
            node = server_fuse_find_node(provider, directory->parent);
            if (!node)
                node = directory;
        }
        else
                                     ...
                                     int close_error)
{
    ...
    if (pending->kind == SERVER_FUSE_PENDING_LOOKUP)
    {
        server_fuse_node* node =
            server_fuse_find_child(provider, pending->parent, pending->name);
    ...
    if (pending->kind == SERVER_FUSE_PENDING_RELEASE ||
...
static void server_fuse_complete_lookup(
    server_fuse* provider, server_fuse_pending* pending,
    const server_platform_drive_completion* completion, int error)
{
    ...
    if (pending->stage == SERVER_FUSE_STAGE_OPEN)
    {
        if (error != 0)
        {
            server_fuse_reply_pending_error(pending, error);
            return;
        }
        pending->temporary_file = completion->file;
        if (!server_fuse_submit_query_directory(provider, pending, parent,
                                                pending->temporary_file,
                                                pending->name, 1))
            server_fuse_reply_pending_error(pending, EIO);
        return;
    }
    if (pending->stage == SERVER_FUSE_STAGE_QUERY)
    {
        pending->terminal_error =
            error != 0
                ? error
                : server_fuse_decode_lookup(provider, pending, completion);
        if (!server_fuse_submit_close(provider, pending, parent,
                                      pending->temporary_file))
            server_fuse_reply_pending_error(pending, pending->terminal_error);
        return;
    }
    server_fuse_finish_close(provider, pending, error);
}

static void server_fuse_complete_open(
    server_fuse* provider, server_fuse_pending* pending,
    const server_platform_drive_completion* completion, int error)
{
    server_fuse_node* node = server_fuse_find_node(provider, pending->inode);
    server_fuse_handle* handle = NULL;

    if (!node)
        error = ESTALE;
    if (pending->stage == SERVER_FUSE_STAGE_CLOSE)
    {
        server_fuse_finish_close(provider, pending, error);
        return;
    }
    if (error == 0)
    {
        handle = server_fuse_allocate_handle(
            provider, pending->inode,
            pending->kind == SERVER_FUSE_PENDING_OPENDIR, 0);
        if (!handle)
            error = EMFILE;
    }
    if (error != 0)
    {
        if (completion->file.file_id != 0u && node)
        {
            ...
            if (server_fuse_submit_close(provider, pending, node,
                                         completion->file))
                return;
        }
        server_fuse_reply_pending_error(pending, error);
    ...
    const server_platform_drive_completion* completion, int error)
{
    server_fuse_handle* handle =
        server_fuse_find_handle(provider, pending->handle_id);
    server_fuse_node* node =
        handle ? server_fuse_find_node(provider, handle->inode) : NULL;
    size_t decoded = 0u;
    ...
    if (error == ENOENT)
    {
        ...
        server_fuse_reply_directory(provider, pending, handle);
        return;
    }
    if (error != 0)
    {
    ...
    error = server_fuse_decode_directory_page(provider, pending, handle,
                                              completion, &decoded);
    if (error != 0 || decoded == 0u)
    {
        server_fuse_reply_pending_error(pending, error != 0 ? error : EPROTO);
        return;
    }
    handle->query_count++;
    if (handle->query_count >= provider->config.max_directory_entries)
    {
        server_fuse_reply_pending_error(pending, ENOSPC);
        return;
    }
    if (!server_fuse_submit_query_directory(provider, pending, node,
    ...
    void* opaque, const server_platform_drive_completion* completion)
{
    ...
    if (!provider || !completion)
        return LIBRDP_STATUS_INVALID_ARGUMENT;
    pending = server_fuse_find_pending(provider, completion->request_id);
    if (!pending)
        return LIBRDP_STATUS_STATE;
    error = server_fuse_completion_errno(completion);
    if (pending->kind == SERVER_FUSE_PENDING_LOOKUP)
        server_fuse_complete_lookup(provider, pending, completion, error);
    else if (pending->kind == SERVER_FUSE_PENDING_OPEN ||
             pending->kind == SERVER_FUSE_PENDING_OPENDIR)
        server_fuse_complete_open(provider, pending, completion, error);
    else if (pending->kind == SERVER_FUSE_PENDING_READ)
    {
...
static void server_fuse_lookup(fuse_req_t request, fuse_ino_t parent_inode,
                               const char* name)
{
    server_fuse* provider = (server_fuse*)fuse_req_userdata(request);
    server_fuse_node* parent = server_fuse_find_node(provider, parent_inode);
    server_fuse_node* child =
        server_fuse_find_child(provider, parent_inode, name);
    server_fuse_pending* pending = NULL;

    if (!parent || !server_fuse_name_valid(name))
    {
    ...
    if (parent->kind == SERVER_FUSE_NODE_ROOT ||
        parent->kind == SERVER_FUSE_NODE_PEER ||
        parent->kind == SERVER_FUSE_NODE_CLIPBOARD_DIRECTORY ||
...
static void server_fuse_forget(fuse_req_t request, fuse_ino_t inode,
                               uint64_t lookup_count)
{
    server_fuse* provider = (server_fuse*)fuse_req_userdata(request);
    server_fuse_node* node = server_fuse_find_node(provider, inode);
...
static void server_fuse_getattr(fuse_req_t request, fuse_ino_t inode,
                                struct fuse_file_info* file_info)
{
    server_fuse* provider = (server_fuse*)fuse_req_userdata(request);
    server_fuse_node* node = server_fuse_find_node(provider, inode);
...
static void server_fuse_open_common(fuse_req_t request, fuse_ino_t inode,
                                    struct fuse_file_info* file_info,
                                    int directory)
{
    server_fuse* provider = (server_fuse*)fuse_req_userdata(request);
    ...
    if (directory != server_fuse_node_is_directory(node))
    {
    ...
    if (node->kind == SERVER_FUSE_NODE_ROOT ||
        ...
        node->kind == SERVER_FUSE_NODE_CLIPBOARD_FILE)
    {
        handle = server_fuse_allocate_handle(provider, inode, directory, 1);
        if (!handle)
        {
    ...
    pending = server_fuse_allocate_pending(
        provider, request,
        directory ? SERVER_FUSE_PENDING_OPENDIR : SERVER_FUSE_PENDING_OPEN);
    if (!pending)
    {
        (void)fuse_reply_err(request, EBUSY);
        return;
...
static void server_fuse_open(fuse_req_t request, fuse_ino_t inode,
                             struct fuse_file_info* file_info)
{
    server_fuse_open_common(request, inode, file_info, 0);
}

static void server_fuse_opendir(fuse_req_t request, fuse_ino_t inode,
                                struct fuse_file_info* file_info)
{
    server_fuse_open_common(request, inode, file_info, 1);
}

/*
 * Translate one kernel read into either a correlated clipboard range request
 * or a remote drive read. Requested lengths are capped before narrowing to
 * the wire type, and the FUSE request remains owned by one pending slot until
 * its terminal completion or cancellation.
 */
static void server_fuse_read(fuse_req_t request, fuse_ino_t inode, size_t size,
                             off_t offset, struct fuse_file_info* file_info)
{
    ...
    if (size == 0u || (node->kind == SERVER_FUSE_NODE_CLIPBOARD_FILE &&
    ...
    if (node->kind == SERVER_FUSE_NODE_CLIPBOARD_FILE)
    {
        ...
        if (!provider->clipboard_request ||
            !server_fuse_clipboard_node_matches(
                node, provider->clipboard_peer_id,
                provider->clipboard_peer_generation,
                provider->clipboard_generation))
        {
            (void)fuse_reply_err(request, ESTALE);
        ...
        pending = server_fuse_allocate_pending(
            provider, request, SERVER_FUSE_PENDING_CLIPBOARD_READ);
        if (!pending)
        {
            (void)fuse_reply_err(request, EBUSY);
            return;
        ...
        pending->request_id = server_fuse_next_request_id(provider);
        ...
        clipboard_request.ownership_generation = node->clipboard_generation;
        ...
        clipboard_request.requested_bytes = (uint32_t)pending->requested_size;
        status = provider->clipboard_request(&clipboard_request,
                                             provider->clipboard_user_data);
        if (status != LIBRDP_STATUS_OK)
            server_fuse_reply_pending_error(pending,
                                            server_fuse_status_errno(status));
        return;
    }
    pending = server_fuse_allocate_pending(provider, request,
                                           SERVER_FUSE_PENDING_READ);
    if (!pending)
    {
        (void)fuse_reply_err(request, EBUSY);
        return;
...
static void server_fuse_reply_local_directory(server_fuse* provider,
                                              fuse_req_t request,
                                              server_fuse_handle* handle,
                                              size_t size, off_t offset)
{
    server_fuse_pending* pending = server_fuse_allocate_pending(
        provider, request, SERVER_FUSE_PENDING_READDIR);
    uint32_t index = 0u;
    ...
    for (index = 0u; index < provider->config.max_nodes; index++)
    {
        ...
        if (!child->valid || child->parent != handle->inode)
            continue;
        if (!server_fuse_handle_add_directory_entry(provider, handle,
                                                    child->inode))
        {
            server_fuse_reply_pending_error(pending, ENOSPC);
            return;
...
static void server_fuse_readdir(fuse_req_t request, fuse_ino_t inode,
                                size_t size, off_t offset,
                                struct fuse_file_info* file_info)
{
    server_fuse* provider = (server_fuse*)fuse_req_userdata(request);
    ...
    if (handle->local)
    {
        server_fuse_reply_local_directory(provider, request, handle, size,
                                          offset);
        return;
    }
...
static void server_fuse_release_common(fuse_req_t request, fuse_ino_t inode,
                                       struct fuse_file_info* file_info,
                                       int directory)
{
    server_fuse* provider = (server_fuse*)fuse_req_userdata(request);
    ...
    pending =
        server_fuse_allocate_pending(provider, request,
                                     directory ? SERVER_FUSE_PENDING_RELEASEDIR
                                               : SERVER_FUSE_PENDING_RELEASE);
    if (!pending)
    {
        (void)fuse_reply_err(request, EBUSY);
        return;
...
static void server_fuse_release(fuse_req_t request, fuse_ino_t inode,
                                struct fuse_file_info* file_info)
{
    server_fuse_release_common(request, inode, file_info, 0);
}

static void server_fuse_releasedir(fuse_req_t request, fuse_ino_t inode,
                                   struct fuse_file_info* file_info)
{
    server_fuse_release_common(request, inode, file_info, 1);
}

static void server_fuse_access(fuse_req_t request, fuse_ino_t inode, int mask)
{
    server_fuse* provider = (server_fuse*)fuse_req_userdata(request);
    server_fuse_node* node = server_fuse_find_node(provider, inode);
...
static const struct fuse_lowlevel_ops server_fuse_operations = {
    ...
    .releasedir = server_fuse_releasedir,
    .statfs = server_fuse_statfs,
    .access = server_fuse_access,
};

static librdp_status server_fuse_events_get_pollfds(void* opaque,
                                                    struct pollfd* fds,
                                                    size_t capacity,
                                                    size_t* count)
{
...
static librdp_status server_fuse_events_notify_poll(void* opaque,
...
static librdp_status server_fuse_events_dispatch(void* opaque,
...
static librdp_status server_fuse_events_get_next_timeout(void* opaque,
                                                         int* timeout_ms)
{
    server_fuse* provider = (server_fuse*)opaque;
...
static const server_platform_event_source_vtable server_fuse_event_source = {
    SERVER_PLATFORM_EVENT_SOURCE_VERSION,
    sizeof(server_platform_event_source_vtable),
    server_fuse_events_get_pollfds,
    server_fuse_events_notify_poll,
    server_fuse_events_dispatch,
    server_fuse_events_get_next_timeout,
};

static void server_fuse_abort_pending(server_fuse* provider, int error)
{
    uint32_t index = 0u;
    ...
    for (index = 0u; index < provider->config.max_pending; index++)
    {
        ...
        if (pending->kind == SERVER_FUSE_PENDING_CLIPBOARD_READ)
        {
            ...
            {
                (void)provider->clipboard_cancel(
                    node->peer_id, node->generation, node->clipboard_generation,
                    pending->request_id, provider->clipboard_user_data);
            }
            if (pending->occupied)
                server_fuse_reply_pending_error(pending, error);
            continue;
        ...
        {
            server_fuse_node* node = server_fuse_find_node(
                provider,
                pending->inode != 0u ? pending->inode : pending->parent);
            server_fuse_node* volume =
                server_fuse_volume_for_node(provider, node);
...
static librdp_status server_fuse_start(void* opaque,
                                       const server_platform_drive_sink* sink)
{
    ...
    {
        provider->session =
            fuse_session_new(&arguments, &server_fuse_operations,
                             sizeof(server_fuse_operations), provider);
...
static void server_fuse_stop(void* opaque)
{
    server_fuse* provider = (server_fuse*)opaque;
    uint32_t index = 0u;
    ...
    if (!provider || !provider->started)
        return;
    server_fuse_abort_pending(provider, ESHUTDOWN);
    if (provider->clipboard_generation != 0u)
    {
        server_fuse_clipboard_clear_internal(
            provider, provider->clipboard_peer_id,
    ...
    for (index = 0u; index < provider->config.max_handles; index++)
        server_fuse_clear_handle(&provider->handles[index]);
    if (provider->session)
    {
...
static server_fuse_node* server_fuse_ensure_peer(server_fuse* provider,
                                                 uint32_t peer_id,
                                                 uint32_t generation)
{
    uint32_t index = 0u;
    ...
    for (index = 0u; index < provider->config.max_nodes; index++)
    {
        ...
        if (node->valid && node->kind == SERVER_FUSE_NODE_PEER &&
            node->peer_id == peer_id && node->generation == generation)
            return node;
    }
    ...
    if (length <= 0 || (size_t)length >= sizeof(name))
        return NULL;
    {
        server_fuse_node* node = server_fuse_allocate_node(
            provider, SERVER_FUSE_NODE_PEER, FUSE_ROOT_ID, name, "");
...
static librdp_status server_fuse_present(
    void* opaque, const server_platform_drive_volume* volume)
{
    server_fuse* provider = (server_fuse*)opaque;
    server_fuse_node* peer = NULL;
    server_fuse_node* node = NULL;
    char visible_name[SERVER_FUSE_MAX_NAME_BYTES + 1u];
    int length = 0;
    ...
    if (!provider || !volume || volume->volume_id == 0u ||
        ...
        return LIBRDP_STATUS_INVALID_ARGUMENT;
    peer =
        server_fuse_ensure_peer(provider, volume->peer_id, volume->generation);
    if (!peer)
        return LIBRDP_STATUS_LIMIT_EXCEEDED;
    if (server_fuse_find_child(provider, peer->inode, volume->name))
    {
    ...
    node = server_fuse_allocate_node(provider, SERVER_FUSE_NODE_VOLUME,
 ...
 */
static void server_fuse_abort_matching(server_fuse* provider, uint32_t peer_id,
...
static void server_fuse_remove_matching(server_fuse* provider, uint32_t peer_id,
                                        uint32_t generation, uint32_t device_id,
                                        int all_peer)
{
    uint32_t index = 0u;
    ...
    if (!provider)
        return;
    server_fuse_abort_matching(provider, peer_id, generation, device_id,
                               all_peer);
    for (index = 0u; index < provider->config.max_handles; index++)
    {
        server_fuse_handle* handle = &provider->handles[index];
    ...
    for (index = 0u; index < provider->config.max_nodes; index++)
    {
        ...
        if (!node->valid || node->kind == SERVER_FUSE_NODE_ROOT ||
            node->kind == SERVER_FUSE_NODE_PEER || node->peer_id != peer_id ||
            node->generation != generation ||
...
static void server_fuse_remove(void* opaque, uint32_t peer_id,
                               uint32_t generation, uint32_t device_id)
{
    server_fuse_remove_matching((server_fuse*)opaque, peer_id, generation,
                                device_id, 0);
}

static void server_fuse_remove_peer(void* opaque, uint32_t peer_id,
                                    uint32_t generation)
{
    server_fuse_remove_matching((server_fuse*)opaque, peer_id, generation, 0u,
                                1);
}

static const server_platform_drive_vtable server_fuse_drive_vtable = {
    ...
    server_fuse_present,
    server_fuse_remove,
    server_fuse_remove_peer,
    server_fuse_complete,
    &server_fuse_event_source,
};
...
void server_fuse_config_init(server_fuse_config* config)
{
    ...
    config->version = SERVER_FUSE_CONFIG_VERSION;
    config->size = sizeof(*config);
    config->max_nodes = SERVER_FUSE_DEFAULT_MAX_NODES;
    config->max_handles = SERVER_FUSE_DEFAULT_MAX_HANDLES;
...
server_fuse* server_fuse_new(const server_fuse_config* config)
{
    ...
    server_fuse_node* root = NULL;

    if (!server_fuse_config_valid(config) ||
        !server_fuse_mount_path_secure(config->mount_path))
        return NULL;
    provider = (server_fuse*)calloc(1u, sizeof(*provider));
    if (!provider)
        return NULL;
    provider->nodes =
        (server_fuse_node*)calloc(config->max_nodes, sizeof(*provider->nodes));
    provider->handles = (server_fuse_handle*)calloc(config->max_handles,
                                                    sizeof(*provider->handles));
    provider->pending = (server_fuse_pending*)calloc(
        config->max_pending, sizeof(*provider->pending));
    provider->mount_path = server_fuse_copy_string(config->mount_path);
    ...
    root->kind = SERVER_FUSE_NODE_ROOT;
    root->valid = 1;
    root->name = server_fuse_copy_string("/");
    root->path = server_fuse_copy_string("");
...
{
    return &server_fuse_drive_vtable;
}
...
librdp_status server_fuse_clipboard_publish(
    ...
    size_t descriptors_len, uint8_t** uri_list, size_t* uri_list_len)
{
    return server_fuse_clipboard_publish_internal(
        provider, peer_id, generation, ownership_generation, descriptors,
        descriptors_len, uri_list, uri_list_len, 1);
}
    ...
    server_fuse* provider, const server_platform_clipboard_data* data)
{
    ...
    if (!provider || !data || data->request_id == 0u || data->stream_id == 0u)
        return LIBRDP_STATUS_INVALID_ARGUMENT;
    pending = server_fuse_find_pending(provider, data->request_id);
    if (!pending || pending->kind != SERVER_FUSE_PENDING_CLIPBOARD_READ)
        return LIBRDP_STATUS_STATE;
    node = server_fuse_find_node(provider, pending->inode);
    if (!server_fuse_clipboard_node_matches(node, data->peer_id,
                                            data->generation,
    ...
    {
        server_fuse_reply_pending_error(pending,
                                        server_fuse_status_errno(data->status));
        return data->status;
    }
    ...
    (void)fuse_reply_buf(pending->request, (const char*)data->data,
                         data->data_len);
    server_fuse_clear_pending(pending);
    return LIBRDP_STATUS_OK;
}
...
{
    return server_fuse_present(provider, volume);
}
...
size_t server_fuse_test_volume_count(const server_fuse* provider)
{
    ...
    {
        if (provider->nodes[index].valid &&
            provider->nodes[index].kind == SERVER_FUSE_NODE_VOLUME)
            count++;
    }
    return count;
}
...
{
    return server_fuse_mount_path_secure(path);
}
    ...
    size_t descriptors_len, uint8_t** uri_list, size_t* uri_list_len)
{
    return server_fuse_clipboard_publish_internal(
        provider, peer_id, generation, ownership_generation, descriptors,
        descriptors_len, uri_list, uri_list_len, 0);
}
...
size_t server_fuse_test_clipboard_file_count(const server_fuse* provider)
{
    ...
    {
        if (provider->nodes[index].valid &&
            provider->nodes[index].kind == SERVER_FUSE_NODE_CLIPBOARD_FILE)
            count++;
    }
    return count;
}
    ...
    size_t descriptors_len, uint8_t** uri_list, size_t* uri_list_len)
{
    return server_fuse_clipboard_publish(
        provider, peer_id, generation, ownership_generation, descriptors,
        descriptors_len, uri_list, uri_list_len);
}
