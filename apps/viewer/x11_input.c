...
static uint32_t x11_input_button_mask(
...
void x11_input_handle_button(x11_app* app, XButtonEvent* button, librdp_mouse_state state)
{
    ...
    button_mask = x11_input_button_mask(event.button);
    if (button_mask != 0u)
    {
        if (state == LIBRDP_MOUSE_PRESSED)
        {
            if ((app->pressed_mouse_buttons &
                 button_mask) != 0u)
                return;
            app->pressed_mouse_buttons |= button_mask;
        }
        else
        {
            if ((app->pressed_mouse_buttons &
...
/*
 * Release stateful buttons before the viewer loses control of local input.
 * Wheel transitions are stateless and never enter the pressed-button mask.
 * Clearing each bit before dispatch prevents a later X11 release generated by
 * the implicit pointer grab from duplicating the remote transition.
 */
void x11_input_release_all_remote_buttons(x11_app* app)
{
    static const librdp_mouse_button buttons[] = {
        LIBRDP_MOUSE_BUTTON_LEFT,
        LIBRDP_MOUSE_BUTTON_RIGHT,
        LIBRDP_MOUSE_BUTTON_MIDDLE,
        LIBRDP_MOUSE_BUTTON_X1,
        LIBRDP_MOUSE_BUTTON_X2
    };
    size_t index = 0u;
         ...
         index++)
    {
        ...
        uint32_t button_mask =
            x11_input_button_mask(buttons[index]);
        ...
        event.x = app->mouse_x;
        event.y = app->mouse_y;
        event.button = buttons[index];
        event.state = LIBRDP_MOUSE_RELEASED;
        if (app->session)
            (void)librdp_session_send_mouse(
                app->session,
                &event);
        released++;
    }
