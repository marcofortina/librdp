...
typedef struct cocoa_server_key_mapping
{
    uint8_t scancode;
    uint8_t extended;
    CGKeyCode keycode;
} cocoa_server_key_mapping;

static const cocoa_server_key_mapping cocoa_server_key_mappings[] = {
    { 0x01u, 0u, kVK_Escape },
    { 0x02u, 0u, kVK_ANSI_1 },
    { 0x03u, 0u, kVK_ANSI_2 },
    { 0x04u, 0u, kVK_ANSI_3 },
                                         ...
                                         CGKeyCode* keycode)
{
    ...
    if (!keycode || scancode == 0u || scancode > 0xffu ||
        ...
        return 0;
    extended =
        (flags & COCOA_RDP_KBD_EXTENDED) != 0u ? 1u : 0u;
    for (index = 0u;
         index < sizeof(cocoa_server_key_mappings) /
                     sizeof(cocoa_server_key_mappings[0]);
         index++)
    {
        if (cocoa_server_key_mappings[index].scancode ==
                (uint8_t)scancode &&
            cocoa_server_key_mappings[index].extended == extended)
        {
            *keycode = cocoa_server_key_mappings[index].keycode;
            return 1;
        }
    }
    return 0;
