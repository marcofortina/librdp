                                         ...
                                         size_t data_len)
{
    ...
    if (!buffer || (!data && data_len > 0))
        return LIBRDP_STATUS_INVALID_ARGUMENT;
    header.flags = RDP_UDP2_FLAG_DATA;
    header.log_window_size = log_window_size;
    status = rdp_udp2_write_header(buffer, &header);
    if (status != LIBRDP_STATUS_OK)
        return status;
    status = rdp_buffer_append_u16_le(buffer, data_sequence_number);
    if (status != LIBRDP_STATUS_OK)
        return status;
                                        ...
                                        uint8_t delayed_ack_time_scale)
{
    ...
    if (!buffer || received_timestamp > 0x00ffffffu ||
        ...
        return LIBRDP_STATUS_INVALID_ARGUMENT;
    header.flags = RDP_UDP2_FLAG_ACK;
    header.log_window_size = log_window_size;
    status = rdp_udp2_write_header(buffer, &header);
    if (status != LIBRDP_STATUS_OK)
        return status;
    ...
    if (status != LIBRDP_STATUS_OK)
        return status;
    status = rdp_udp_append_u24_le(buffer, received_timestamp);
    if (status != LIBRDP_STATUS_OK)
        return status;
    status = rdp_buffer_append_u8(buffer, send_ack_time_gap_ms);
    if (status != LIBRDP_STATUS_OK)
        return status;
    packed = (uint8_t)(delayed_ack_count | (uint8_t)(delayed_ack_time_scale << 4));
    status = rdp_buffer_append_u8(buffer, packed);
    if (status != LIBRDP_STATUS_OK)
        return status;
    return rdp_buffer_append(buffer, delayed_ack_time_additions, delayed_ack_count);
}
