...
#define RDP_SESSION_CTAPHID_REPORT_LENGTH 64u
#define RDP_SESSION_CTAPHID_INIT_PAYLOAD_LENGTH 57u
#define RDP_SESSION_CTAPHID_CONT_PAYLOAD_LENGTH 59u
#define RDP_SESSION_CTAPHID_MAX_PAYLOAD (57u + 128u * 59u)
#define RDP_SESSION_CTAPHID_BROADCAST_CID 0xffffffffu
#define RDP_SESSION_CTAPHID_CMD_INIT 0x86u
#define RDP_SESSION_CTAPHID_CMD_CBOR 0x90u
#define RDP_SESSION_CTAPHID_CMD_KEEPALIVE 0xbbu
#define RDP_SESSION_CTAPHID_CMD_ERROR 0xbfu
#define RDP_SESSION_CTAPHID_TIMEOUT_MS 60000u
                                                                ...
                                                                size_t data_len)
{
    ...
    switch (server_header.function_id)
    {
        ...
        case RDP_PNP_REDIRECTION_IO_CREATE_FILE_REQUEST:
        {
            ...
            if (status == LIBRDP_STATUS_OK)
            {
                ...
                if (status == LIBRDP_STATUS_OK)
                    rdp_trace_event(RDP_TRACE_CLIENT,
                                    "client.pnp.create_reply",
                                    "request_id=%u device_id=%u status=%u",
                                    request.header.request_id,
                                    request.device_id,
                                    io_status);
            }
        ...
        case RDP_PNP_REDIRECTION_IO_READ_REQUEST:
        {
            ...
            if (status == LIBRDP_STATUS_OK)
            {
                rdp_buffer read_data;
                uint32_t index = 0;
                uint32_t io_status = rdp_session_pnp_open_device_index(session, &index);
                const uint8_t* payload = NULL;
                uint32_t payload_len = 0;

                rdp_buffer_init(&read_data);
                if (io_status == RDP_DEVICE_REDIRECTION_STATUS_SUCCESS)
                {
                    status = rdp_session_pnp_build_read_data(session, index, &read_data);
                    if (status != LIBRDP_STATUS_OK)
                    ...
                    else
                    {
                        ...
                        if (offset < read_data.length)
                        {
                            size_t available = read_data.length - (size_t)offset;
                            size_t requested = request.bytes_to_read;
                            if (requested > available)
                                requested = available;
                            payload = read_data.data + (size_t)offset;
                            payload_len = (uint32_t)requested;
                        }
                    }
                }
        ...
        case RDP_PNP_REDIRECTION_IO_CONTROL_REQUEST:
        {
            ...
            if (status == LIBRDP_STATUS_OK)
            {
                ...
                {
                    (void)index;
                    if (request.io_code != 0 || request.input_len != 0 || request.output_len != 0)
                ...
                if (status == LIBRDP_STATUS_OK)
                    rdp_trace_event(RDP_TRACE_CLIENT,
                                    "client.pnp.control_reply",
                                    "request_id=%u status=%u io_code=%u input_len=%u output_len=%u",
                                    request.header.request_id,
                                    io_status,
                                    request.io_code,
                                    request.input_len,
                                                            ...
                                                            const rdp_session_redirected_file* file)
{
    ...
    rdp_buffer_init(&utf16);
    status = rdp_session_utf8_to_utf16le(name, &utf16, 0);
    if (status == LIBRDP_STATUS_OK && utf16.length > UINT32_MAX - 94u)
        status = LIBRDP_STATUS_NO_MEMORY;
    if (status == LIBRDP_STATUS_OK)
        status = rdp_session_current_file_position(file->fd, &position);
    change_time = rdp_session_stat_ctime(st);
    if (status == LIBRDP_STATUS_OK)
        status = rdp_buffer_append_u32_le(buffer, 94u + (uint32_t)utf16.length);
    if (status == LIBRDP_STATUS_OK)
        status = rdp_session_append_u64_le(buffer, change_time);
    if (status == LIBRDP_STATUS_OK)
        status = rdp_session_append_u64_le(buffer, rdp_session_stat_atime(st));
...
static uint32_t rdp_session_apply_valid_data_length_information(rdp_session_redirected_file* file,
                                                                const uint8_t* data,
                                                                uint32_t length)
{
    struct stat st;
...
static uint32_t rdp_session_apply_position_information(rdp_session_redirected_file* file,
                                                       const uint8_t* data,
                                                       uint32_t length)
{
    uint64_t position = 0;
...
static uint32_t rdp_session_apply_mode_information(rdp_session_redirected_file* file,
                                                   const uint8_t* data,
                                                   uint32_t length)
{
    ...
    else
    {
        switch (request.io_control_code)
        {
            ...
            case RDP_FILESYSTEM_REDIRECTION_FSCTL_SET_COMPRESSION:
                if (request.input_buffer_length != 2u || !request.input_buffer)
                    io_status = RDP_SESSION_DEVICE_INVALID_PARAMETER;
    ...
    const rdp_smartcard_redirection_request_message* message)
{
    ...
    if (context)
    {
        ...
        else
        {
            ...
            if (pcsc_status == SCARD_S_SUCCESS)
                pcsc_status = SCardGetStatusChange(context->context,
...
} rdp_session_webauthn_fido2_device;

static void rdp_session_webauthn_copy_text(char* dst, size_t dst_len, const char* src, const char* fallback)
{
...
static void rdp_session_webauthn_init_fido2_info(rdp_session_webauthn_fido2_device* device)
{
    ...
    rdp_session_webauthn_copy_text(device->manufacturer,
                                   sizeof(device->manufacturer),
                                   NULL,
                                   "FIDO2");
    rdp_session_webauthn_copy_text(device->product,
    ...
    device->info.max_msg_size = RDP_SESSION_CTAPHID_MAX_PAYLOAD;
...
static void rdp_session_ctaphid_write_u32_be(uint8_t* data, uint32_t value)
{
...
static uint64_t rdp_session_ctaphid_time_ms(void)
{
    struct timespec ts;
...
static int rdp_session_ctaphid_write_frame(int fd, const uint8_t* frame)
{
    ssize_t written = 0;
    ...
    do
    {
        written = write(fd, frame, RDP_SESSION_CTAPHID_REPORT_LENGTH);
    } while (written < 0 && errno == EINTR);
    return written == (ssize_t)RDP_SESSION_CTAPHID_REPORT_LENGTH;
}

static int rdp_session_ctaphid_read_frame(int fd, uint8_t* frame, uint64_t deadline_ms)
{
    for (;;)
    {
        struct pollfd pfd;
        uint64_t now_ms = rdp_session_ctaphid_time_ms();
        int timeout = 0;
        ssize_t count = 0;
        ...
        if ((pfd.revents & POLLIN) == 0)
            return 0;
        count = read(fd, frame, RDP_SESSION_CTAPHID_REPORT_LENGTH);
        if (count < 0 && (errno == EINTR || errno == EAGAIN))
            continue;
        return count == (ssize_t)RDP_SESSION_CTAPHID_REPORT_LENGTH;
    }
}

static librdp_status rdp_session_ctaphid_send(uint32_t cid,
                                              uint8_t command,
                                              const uint8_t* payload,
                                              size_t payload_len,
                                              int fd)
{
    uint8_t frame[RDP_SESSION_CTAPHID_REPORT_LENGTH];
    size_t offset = 0;
    size_t chunk = 0;
    uint8_t seq = 0;

    if (fd < 0 || (!payload && payload_len > 0) || payload_len > RDP_SESSION_CTAPHID_MAX_PAYLOAD)
        return LIBRDP_STATUS_INVALID_ARGUMENT;
    memset(frame, 0, sizeof(frame));
    rdp_session_ctaphid_write_u32_be(frame, cid);
    frame[4] = command;
    frame[5] = (uint8_t)((payload_len >> 8) & 0xffu);
    frame[6] = (uint8_t)(payload_len & 0xffu);
    chunk = payload_len < RDP_SESSION_CTAPHID_INIT_PAYLOAD_LENGTH ?
                payload_len :
                RDP_SESSION_CTAPHID_INIT_PAYLOAD_LENGTH;
    if (chunk > 0)
        memcpy(frame + 7u, payload, chunk);
    if (!rdp_session_ctaphid_write_frame(fd, frame))
    ...
    while (offset < payload_len)
    {
        ...
        rdp_session_ctaphid_write_u32_be(frame, cid);
        frame[4] = seq++;
        chunk = remaining < RDP_SESSION_CTAPHID_CONT_PAYLOAD_LENGTH ?
                    remaining :
                    RDP_SESSION_CTAPHID_CONT_PAYLOAD_LENGTH;
...
static librdp_status rdp_session_ctaphid_recv(uint32_t cid,
...
{
    uint8_t frame[RDP_SESSION_CTAPHID_REPORT_LENGTH];
    uint64_t deadline = rdp_session_ctaphid_time_ms() + timeout_ms;
    uint16_t total = 0;
    size_t copied = 0;
    uint8_t seq = 0;
    librdp_status status = LIBRDP_STATUS_OK;
    ...
    for (;;)
    {
        ...
        if (!rdp_session_ctaphid_read_frame(fd, frame, deadline))
            return LIBRDP_STATUS_TIMEOUT;
        frame_cid = rdp_session_ctaphid_read_u32_be(frame);
        command = frame[4];
        if (frame_cid != cid)
            continue;
        if (command == RDP_SESSION_CTAPHID_CMD_KEEPALIVE)
        {
            rdp_trace_event_level(RDP_TRACE_CLIENT,
                                  RDP_TRACE_LEVEL_DEBUG,
        ...
        if (command == RDP_SESSION_CTAPHID_CMD_ERROR)
        {
            rdp_trace_event(RDP_TRACE_CLIENT,
    ...
    while (copied < total)
    {
        ...
        if (frame_cid != cid)
            continue;
        if (frame[4] == RDP_SESSION_CTAPHID_CMD_KEEPALIVE)
            continue;
...
static librdp_status rdp_session_ctaphid_init(int fd, uint32_t* cid)
{
    uint8_t nonce[8];
    rdp_buffer response;
    librdp_status status = LIBRDP_STATUS_OK;
    ...
    if (RAND_bytes(nonce, sizeof(nonce)) != 1)
        return LIBRDP_STATUS_STATE;
    rdp_buffer_init(&response);
    status = rdp_session_ctaphid_send(RDP_SESSION_CTAPHID_BROADCAST_CID,
                                      RDP_SESSION_CTAPHID_CMD_INIT,
                                      nonce,
                                      sizeof(nonce),
                                      fd);
    if (status == LIBRDP_STATUS_OK)
        status = rdp_session_ctaphid_recv(RDP_SESSION_CTAPHID_BROADCAST_CID,
                                          RDP_SESSION_CTAPHID_CMD_INIT,
                                          fd,
                                          &response,
                                          5000u);
    ...
    if (status == LIBRDP_STATUS_OK)
    {
        *cid = rdp_session_ctaphid_read_u32_be(response.data + 8u);
        rdp_trace_event(RDP_TRACE_CLIENT,
...
static void rdp_session_webauthn_fill_fido2_cbor_info(rdp_session_webauthn_fido2_device* device)
{
    fido_dev_t* dev = NULL;
...
static librdp_status rdp_session_webauthn_select_fido2_device(const librdp_session* session,
                                                              rdp_session_webauthn_fido2_device* device)
{
    ...
    if (!device)
        return LIBRDP_STATUS_INVALID_ARGUMENT;
    rdp_session_webauthn_init_fido2_info(device);
    fido_init(0);
    ...
    if (list && fido_dev_info_manifest(list, 64, &found) == FIDO_OK)
    {
        for (i = 0; i < found; i++)
        {
            ...
            if (!path || (requested_path && strcmp(requested_path, path) != 0))
                continue;
            rdp_session_webauthn_copy_text(device->path, sizeof(device->path), path, "");
            rdp_session_webauthn_copy_text(device->manufacturer,
                                           sizeof(device->manufacturer),
    ...
    {
        rdp_session_webauthn_copy_text(device->path, sizeof(device->path), requested_path, "");
        selected = 1;
    }
    ...
    rdp_session_webauthn_fill_fido2_cbor_info(device);
    rdp_trace_event(RDP_TRACE_CLIENT,
...
static librdp_status rdp_session_webauthn_fido2_exchange(const librdp_session* session,
                                                         ...
                                                         rdp_buffer* ctap_response)
{
    ...
    if (!session || !request || !device || !ctap_response || !request->request ||
        request->request_len == 0 || request->request_len > RDP_SESSION_CTAPHID_MAX_PAYLOAD)
        return LIBRDP_STATUS_INVALID_ARGUMENT;
    status = rdp_session_webauthn_select_fido2_device(session, device);
    if (status != LIBRDP_STATUS_OK)
        return status;
    ...
    status = rdp_session_ctaphid_init(fd, &cid);
    if (status == LIBRDP_STATUS_OK)
        status = rdp_session_ctaphid_send(cid,
                                          RDP_SESSION_CTAPHID_CMD_CBOR,
                                          request->request,
                                          request->request_len,
                                          fd);
    if (status == LIBRDP_STATUS_OK)
        status = rdp_session_ctaphid_recv(cid,
                                          RDP_SESSION_CTAPHID_CMD_CBOR,
                                          fd,
                                          ctap_response,
                                          RDP_SESSION_CTAPHID_TIMEOUT_MS);
    close(fd);
                                                         ...
                                                         size_t data_len)
{
    ...
    else if (request.command == RDP_WEBAUTHN_COMMAND_WEB_AUTHN && fido2_enabled)
    {
        ...
        fido2_status = rdp_session_webauthn_fido2_exchange(session, &request, &device, &ctap_response);
...
static void rdp_session_usb_redirection_reset(librdp_session* session)
{
    ...
    for (i = 0; i < LIBRDP_SETTINGS_MAX_USB_DEVICES; i++)
    {
        ...
        {
            for (size_t j = 0; j < sizeof(session->usb_devices[i].claimed_interfaces); j++)
            {
                if (session->usb_devices[i].claimed_interfaces[j])
                    (void)libusb_release_interface(session->usb_devices[i].handle, (int)j);
            }
...
static rdp_session_usb_device* rdp_session_usb_device_by_interface_mut(librdp_session* session,
                                                                       uint32_t interface_id)
{
    size_t i = 0;
...
static uint32_t rdp_session_usb_libusb_status(int rc)
{
    switch (rc)
    {
        case LIBUSB_SUCCESS:
...
#ifdef RDP_HAVE_LIBUSB
    {
                 ...
                 transfer->urb.function == RDP_USB_REDIRECTION_URB_CONTROL_TRANSFER_EX)
        {
            ...
            else if (control.output_buffer_size > 0)
            {
                ...
                else
                {
                    ...
                    actual = libusb_control_transfer(device->handle,
                                                    control.request_type,
                                                    control.request,
                                                    control.value,
                                                    control.index,
                                                    output,
                                                    (uint16_t)control.output_buffer_size,
                                                    control.timeout);
                    if (actual >= 0)
                    {
                        usbd_status = RDP_USB_REDIRECTION_USBD_STATUS_SUCCESS;
                        if ((control.request_type & LIBUSB_ENDPOINT_IN) != 0)
                            output_len = (uint32_t)actual;
                    }
                    else
                    {
                        usbd_status = rdp_session_usb_libusb_status(actual);
                    }
                    rdp_trace_event(RDP_TRACE_CLIENT,
                                    "client.urbdrc.control_transfer",
                                    "interface_id=%u endpoint=%u request=%u value=%u index=%u flags=%u direction=%u requested=%u actual=%d usbd_status=%u",
            ...
            {
                int actual = libusb_control_transfer(device->handle,
                                                    control.request_type,
                                                    control.request,
                                                    control.value,
                                                    control.index,
        ...
        else if (transfer->urb.function == RDP_USB_REDIRECTION_URB_BULK_OR_INTERRUPT_TRANSFER)
        {
            ...
            else
            {
                ...
                usbd_status = rdp_session_usb_claim_endpoint(device, endpoint, &transfer_type);
                    ...
                    if (transfer->header.function_id == RDP_USB_REDIRECTION_FN_TRANSFER_IN_REQUEST)
                    {
                        ...
                        if (usbd_status == RDP_USB_REDIRECTION_USBD_STATUS_SUCCESS)
                        {
                            if (transfer_type == LIBUSB_TRANSFER_TYPE_INTERRUPT)
                                rc = libusb_interrupt_transfer(device->handle,
                                                               endpoint,
                                                               output,
                                                               (int)pipe.output_buffer_size,
                                                               &actual,
                                                               10000u);
                            else
                            ...
                            if (rc == LIBUSB_SUCCESS)
                            {
                                ...
                                usbd_status = rdp_session_usb_libusb_status(rc);
                        }
                    }
                    else
                    {
                        if (transfer_type == LIBUSB_TRANSFER_TYPE_INTERRUPT)
                            ...
                            rc = libusb_bulk_transfer(device->handle,
                                                      ...
                                                      &actual,
                                                      10000u);
                        usbd_status = rc == LIBUSB_SUCCESS ? RDP_USB_REDIRECTION_USBD_STATUS_SUCCESS :
                                                             rdp_session_usb_libusb_status(rc);
                    }
                    rdp_trace_event(RDP_TRACE_CLIENT,
                                    "client.urbdrc.pipe_transfer",
                                    "interface_id=%u endpoint=%u type=%u flags=%u direction=%u requested=%u actual=%d usbd_status=%u",
                                    transfer->header.interface_id,
                                    endpoint,
                                    transfer_type,
                                    pipe.transfer_flags,
