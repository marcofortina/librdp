                                                        ...
                                                        uint16_t bottom)
{
    ...
    if (status == LIBRDP_STATUS_OK)
    {
        ...
        event.data.surface.x = dst_x;
        event.data.surface.y = dst_y;
                                                                  ...
                                                                  uint32_t* failed_tiles)
{
    ...
    if (!session || !surface || !region || !rendered_tiles || !failed_tiles)
        return LIBRDP_STATUS_INVALID_ARGUMENT;
    ...
    if (status != LIBRDP_STATUS_OK)
    {
        ...
        rdp_trace_event(RDP_TRACE_CLIENT,
                        ...
                        "dvc_channel_id=%u surface_id=%u x=%u y=%u block_type=%u status=%d y_len=%u cb_len=%u cr_len=%u extrapolate=%u",
                        channel_id,
                        surface->surface_id,
                        x,
                        y,
    ...
    while (offset < decoded.length)
    {
        ...
        else if (header.cmd_id == RDP_GRAPHICS_CMDID_RESET_GRAPHICS)
        {
                ...
                reset.height != librdp_surface_height(session->surface))
            {
                ...
                event.data.surface.width = reset.width;
                event.data.surface.height = reset.height;
                rdp_session_emit(session, &event);
            }
            rdp_trace_event(RDP_TRACE_CLIENT,
...
librdp_status librdp_session_send_key(librdp_session* session, const librdp_key_event* key)
{
    ...
    rdp_trace_event(RDP_TRACE_CLIENT,
                    "client.input.send",
                    "kind=keyboard scancode=%u transport=%s flags=%u",
                    key->scancode,
                    session->core_input_ready ? "core_input" : "slowpath",
                    session->core_input_ready ? (key->state == LIBRDP_KEY_RELEASED ? 1u : 0u) : flags);
    rdp_buffer_free(&input);
    return LIBRDP_STATUS_OK;
}
