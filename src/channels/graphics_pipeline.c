 ...
 */
/*
 * Module: graphics pipeline channel segmentation, capabilities, and surface
 * commands.
...
/*
 * Decode one graphics-pipeline compressed segment into a caller buffer.
 * Compression headers, expanded size, and history state are validated before
 * any decoded bytes are made visible.
 */
...
librdp_status rdp_graphics_parse_header(const void* data, size_t length, rdp_graphics_header* header)
{
    ...
    if (rdp_stream_read_u16_le(&stream, &parsed.cmd_id) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.flags) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.pdu_length) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (parsed.flags != 0 || parsed.pdu_length < 8u || parsed.pdu_length > length)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *header = parsed;
    return LIBRDP_STATUS_OK;
}
...
librdp_status rdp_graphics_parse_capset(const void* data, size_t length, rdp_graphics_capset* capset)
{
    ...
    if (rdp_stream_read_u32_le(&stream, &parsed.flags) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (!rdp_graphics_capset_valid(&parsed))
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *capset = parsed;
    return LIBRDP_STATUS_OK;
}
...
{
    rdp_graphics_caps_confirm parsed;
    ...
    if (header.cmd_id != RDP_GRAPHICS_CMDID_CAPS_CONFIRM || header.pdu_length != length)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    status = rdp_graphics_parse_capset(((const uint8_t*)data) + 8u, length - 8u, &parsed.selected);
    if (status != LIBRDP_STATUS_OK)
        return status;
    *confirm = parsed;
    return LIBRDP_STATUS_OK;
...
{
    rdp_graphics_create_surface parsed;
    rdp_stream stream;
    ...
    if (rdp_stream_skip(&stream, 8) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.surface_id) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.width) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.height) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u8(&stream, &parsed.pixel_format) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (parsed.width == 0 || parsed.height == 0 ||
        !rdp_graphics_pixel_format_supported(parsed.pixel_format))
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *create_surface = parsed;
    return LIBRDP_STATUS_OK;
}
...
{
    rdp_graphics_delete_surface parsed;
    rdp_stream stream;
...
{
    rdp_graphics_map_surface_to_output parsed;
    rdp_stream stream;
    uint16_t reserved = 0;
    ...
    if (rdp_stream_skip(&stream, 8) != LIBRDP_STATUS_OK ||
        ...
        rdp_stream_read_u32_le(&stream, &parsed.output_origin_x) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.output_origin_y) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (reserved != 0)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *map = parsed;
...
{
    rdp_graphics_map_surface_to_scaled_output parsed;
    rdp_stream stream;
    uint16_t reserved = 0;
    ...
    if (rdp_stream_skip(&stream, 8) != LIBRDP_STATUS_OK ||
        ...
        rdp_stream_read_u32_le(&stream, &parsed.output_origin_x) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.output_origin_y) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.target_width) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.target_height) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (reserved != 0 || parsed.target_width == 0 || parsed.target_height == 0)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *map = parsed;
    return LIBRDP_STATUS_OK;
}
...
librdp_status rdp_graphics_parse_rect16(const void* data, size_t length, rdp_graphics_rect16* rect)
{
    ...
    memset(&parsed, 0, sizeof(parsed));
    rdp_stream_init(&stream, data, length);
    if (rdp_stream_read_u16_le(&stream, &parsed.left) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.top) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.right) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.bottom) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
...
{
    rdp_graphics_solid_fill parsed;
    rdp_graphics_header header;
    rdp_stream stream;
    ...
    if (rdp_stream_skip(&stream, 8) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.surface_id) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.fill_pixel) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.rect_count) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if ((size_t)parsed.rect_count != rdp_stream_remaining(&stream) / 8u ||
        rdp_stream_remaining(&stream) % 8u != 0)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (rdp_stream_read_bytes(&stream, &parsed.rects, rdp_stream_remaining(&stream)) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    parsed.rects_len = (size_t)parsed.rect_count * 8u;
    *solid_fill = parsed;
    return LIBRDP_STATUS_OK;
}
...
{
    rdp_graphics_wire_to_surface_1 parsed;
    rdp_graphics_header header;
    rdp_stream stream;
    ...
    if (rdp_stream_skip(&stream, 8) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.surface_id) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.codec_id) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u8(&stream, &parsed.pixel_format) != LIBRDP_STATUS_OK ||
        rdp_stream_read_bytes(&stream, &rect, 8) != LIBRDP_STATUS_OK ||
        rdp_graphics_parse_rect16(rect, 8, &parsed.dest_rect) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.bitmap_data_length) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if ((parsed.pixel_format != RDP_GRAPHICS_PIXEL_FORMAT_XRGB_8888 &&
         parsed.pixel_format != RDP_GRAPHICS_PIXEL_FORMAT_ARGB_8888) ||
        !rdp_graphics_wire_to_surface_1_codec_supported(parsed.codec_id) ||
        parsed.dest_rect.right == parsed.dest_rect.left ||
        parsed.dest_rect.bottom == parsed.dest_rect.top ||
        rdp_stream_remaining(&stream) != parsed.bitmap_data_length)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
...
{
    rdp_graphics_wire_to_surface_2 parsed;
    rdp_graphics_header header;
    rdp_stream stream;
    ...
    if (rdp_stream_skip(&stream, 8) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.surface_id) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.codec_id) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.codec_context_id) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u8(&stream, &parsed.pixel_format) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.bitmap_data_length) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if ((parsed.pixel_format != RDP_GRAPHICS_PIXEL_FORMAT_XRGB_8888 &&
         parsed.pixel_format != RDP_GRAPHICS_PIXEL_FORMAT_ARGB_8888) ||
        !rdp_graphics_wire_to_surface_2_codec_supported(parsed.codec_id) ||
        rdp_stream_remaining(&stream) != parsed.bitmap_data_length)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (rdp_stream_read_bytes(&stream, &parsed.bitmap_data, parsed.bitmap_data_length) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
...
{
    rdp_graphics_surface_to_surface parsed;
    rdp_graphics_header header;
    rdp_stream stream;
    const uint8_t* rect = NULL;
    ...
    rdp_stream_init(&stream, data, length);
    if (rdp_stream_skip(&stream, 8) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.surface_id_src) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.surface_id_dest) != LIBRDP_STATUS_OK ||
        rdp_stream_read_bytes(&stream, &rect, 8) != LIBRDP_STATUS_OK ||
        rdp_graphics_parse_rect16(rect, 8, &parsed.rect_src) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.dest_points_count) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if ((size_t)parsed.dest_points_count != rdp_stream_remaining(&stream) / 4u ||
    ...
    if (rdp_stream_read_bytes(&stream,
        ...
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    parsed.dest_points_len = (size_t)parsed.dest_points_count * 4u;
    *surface_to_surface = parsed;
    return LIBRDP_STATUS_OK;
}
...
{
    rdp_graphics_surface_to_cache parsed;
    rdp_stream stream;
    const uint8_t* rect = NULL;
    ...
    if (rdp_stream_skip(&stream, 8) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.surface_id) != LIBRDP_STATUS_OK ||
        rdp_graphics_stream_read_u64_le(&stream, &parsed.cache_key) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.cache_slot) != LIBRDP_STATUS_OK ||
        rdp_stream_read_bytes(&stream, &rect, 8) != LIBRDP_STATUS_OK ||
        rdp_graphics_parse_rect16(rect, 8, &parsed.rect_src) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *surface_to_cache = parsed;
    return LIBRDP_STATUS_OK;
}
...
{
    rdp_graphics_cache_to_surface parsed;
    rdp_graphics_header header;
    rdp_stream stream;
...
{
    rdp_graphics_evict_cache_entry parsed;
    rdp_stream stream;
...
{
    rdp_graphics_delete_encoding_context parsed;
    rdp_stream stream;
...
{
    rdp_graphics_start_frame parsed;
    rdp_stream stream;
    ...
    rdp_stream_init(&stream, data, length);
    if (rdp_stream_skip(&stream, 8) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.timestamp) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.frame_id) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *start_frame = parsed;
    return LIBRDP_STATUS_OK;
}
                                           ...
                                           rdp_graphics_frame_ack* ack)
{
    ...
    rdp_stream_init(&stream, data, length);
    if (rdp_stream_skip(&stream, 8) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.queue_depth) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.frame_id) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.total_frames_decoded) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *ack = parsed;
    return LIBRDP_STATUS_OK;
}
...
/*
 * Reassemble graphics-pipeline segmented payloads into command data. Segment
 * ordering and total length are checked so later surface decoders never
 * consume incomplete command streams.
 */
...
{
    rdp_graphics_progressive_block parsed;
    rdp_stream stream;
    ...
    if (rdp_stream_read_u16_le(&stream, &parsed.type) != LIBRDP_STATUS_OK ||
        ...
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (parsed.length < 6u || (size_t)parsed.length > length)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (rdp_stream_read_bytes(&stream, &parsed.payload, (size_t)parsed.length - 6u) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    parsed.payload_len = (size_t)parsed.length - 6u;
    *block = parsed;
    return LIBRDP_STATUS_OK;
}
...
{
    rdp_graphics_progressive_context parsed;
    rdp_graphics_progressive_block block;
    rdp_stream stream;
    ...
    if (rdp_stream_read_u8(&stream, &parsed.context_id) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.tile_size) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u8(&stream, &parsed.flags) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (parsed.tile_size != RDP_GRAPHICS_PROGRESSIVE_TILE_SIZE)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *context = parsed;
    return LIBRDP_STATUS_OK;
}
...
{
    rdp_graphics_progressive_frame_begin parsed;
    rdp_graphics_progressive_block block;
    rdp_stream stream;
    ...
    memset(&parsed, 0, sizeof(parsed));
    rdp_stream_init(&stream, block.payload, block.payload_len);
    if (rdp_stream_read_u32_le(&stream, &parsed.frame_index) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.region_count) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (parsed.region_count == 0 ||
        (uint32_t)parsed.region_count > RDP_GRAPHICS_PROGRESSIVE_MAX_REGIONS)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *frame_begin = parsed;
    return LIBRDP_STATUS_OK;
}
...
{
    rdp_graphics_progressive_region parsed;
    rdp_graphics_progressive_block block;
    rdp_stream stream;
    size_t rects_len = 0;
    ...
    memset(&parsed, 0, sizeof(parsed));
    rdp_stream_init(&stream, block.payload, block.payload_len);
    if (rdp_stream_read_u8(&stream, &parsed.tile_size) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.rect_count) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u8(&stream, &parsed.quant_count) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u8(&stream, &parsed.progressive_quant_count) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u8(&stream, &parsed.flags) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.tile_count) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.tile_data_size) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (parsed.tile_size != RDP_GRAPHICS_PROGRESSIVE_TILE_SIZE ||
        parsed.rect_count == 0 ||
        parsed.quant_count == 0 ||
        (uint32_t)parsed.rect_count > RDP_GRAPHICS_PROGRESSIVE_MAX_RECTS ||
        (uint32_t)parsed.tile_count > RDP_GRAPHICS_PROGRESSIVE_MAX_TILES)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if ((parsed.tile_count == 0 && parsed.tile_data_size != 0) ||
        (parsed.tile_count != 0 && parsed.tile_data_size == 0))
        return LIBRDP_STATUS_PROTOCOL_ERROR;

    rects_len = (size_t)parsed.rect_count * 8u;
    quant_values_len = (size_t)parsed.quant_count * 5u;
    progressive_quant_values_len = (size_t)parsed.progressive_quant_count * 16u;
    if (rdp_stream_remaining(&stream) < rects_len + quant_values_len + progressive_quant_values_len ||
        (size_t)parsed.tile_data_size >
            rdp_stream_remaining(&stream) - rects_len - quant_values_len - progressive_quant_values_len)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (rdp_stream_read_bytes(&stream, &parsed.rects, rects_len) != LIBRDP_STATUS_OK ||
        rdp_stream_read_bytes(&stream, &parsed.quant_values, quant_values_len) != LIBRDP_STATUS_OK ||
        rdp_stream_read_bytes(&stream,
                              &parsed.progressive_quant_values,
                              progressive_quant_values_len) != LIBRDP_STATUS_OK ||
        rdp_stream_read_bytes(&stream, &parsed.tiles, parsed.tile_data_size) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (rdp_stream_remaining(&stream) != 0)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    parsed.rects_len = rects_len;
    parsed.quant_values_len = quant_values_len;
    parsed.progressive_quant_values_len = progressive_quant_values_len;
    parsed.tiles_len = (size_t)parsed.tile_data_size;
    *region = parsed;
    return LIBRDP_STATUS_OK;
}
                                                         ...
                                                         rdp_graphics_rect16* rect)
{
    ...
    parsed.left = x;
    parsed.top = y;
    parsed.right = (uint16_t)right;
    parsed.bottom = (uint16_t)bottom;
    *rect = parsed;
    return LIBRDP_STATUS_OK;
}
...
{
    rdp_graphics_progressive_tile_simple parsed;
    rdp_graphics_progressive_block block;
    rdp_stream stream;
    ...
    memset(&parsed, 0, sizeof(parsed));
    rdp_stream_init(&stream, block.payload, block.payload_len);
    parsed.block_type = block.type;
    if (rdp_graphics_progressive_read_tile_prefix(&stream,
                                                  &parsed.quant_idx_y,
                                                  &parsed.quant_idx_cb,
                                                  &parsed.quant_idx_cr,
                                                  &parsed.x_idx,
                                                  &parsed.y_idx) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u8(&stream, &parsed.flags) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.y_len) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.cb_len) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.cr_len) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.tail_len) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    payload_len = (size_t)parsed.y_len + parsed.cb_len + parsed.cr_len + parsed.tail_len;
    if (rdp_stream_remaining(&stream) != payload_len)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (rdp_graphics_progressive_read_tile_data(&stream, parsed.y_len, &parsed.y_data) != LIBRDP_STATUS_OK ||
        rdp_graphics_progressive_read_tile_data(&stream, parsed.cb_len, &parsed.cb_data) != LIBRDP_STATUS_OK ||
        rdp_graphics_progressive_read_tile_data(&stream, parsed.cr_len, &parsed.cr_data) != LIBRDP_STATUS_OK ||
        rdp_graphics_progressive_read_tile_data(&stream, parsed.tail_len, &parsed.tail_data) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *tile = parsed;
    return LIBRDP_STATUS_OK;
}
...
{
    rdp_graphics_progressive_tile_first parsed;
    rdp_graphics_progressive_block block;
    rdp_stream stream;
    size_t payload_len = 0;
    ...
    if (rdp_graphics_progressive_read_tile_prefix(&stream,
                                                  ...
                                                  &parsed.y_idx) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u8(&stream, &parsed.flags) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u8(&stream, &parsed.progressive_quality) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.y_len) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.cb_len) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.cr_len) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.tail_len) != LIBRDP_STATUS_OK)
...
{
    rdp_graphics_progressive_tile_upgrade parsed;
    rdp_graphics_progressive_block block;
    rdp_stream stream;
    size_t payload_len = 0;
    ...
    if (rdp_graphics_progressive_read_tile_prefix(&stream,
                                                  ...
                                                  &parsed.y_idx) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u8(&stream, &parsed.progressive_quality) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.y_srl_len) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.y_raw_len) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.cb_srl_len) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.cb_raw_len) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.cr_srl_len) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.cr_raw_len) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    payload_len = (size_t)parsed.y_srl_len + parsed.y_raw_len + parsed.cb_srl_len +
                  parsed.cb_raw_len + parsed.cr_srl_len + parsed.cr_raw_len;
    if (rdp_stream_remaining(&stream) != payload_len)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (rdp_graphics_progressive_read_tile_data(&stream, parsed.y_srl_len, &parsed.y_srl_data) !=
            LIBRDP_STATUS_OK ||
        rdp_graphics_progressive_read_tile_data(&stream, parsed.y_raw_len, &parsed.y_raw_data) !=
            LIBRDP_STATUS_OK ||
        rdp_graphics_progressive_read_tile_data(&stream, parsed.cb_srl_len, &parsed.cb_srl_data) !=
            LIBRDP_STATUS_OK ||
        rdp_graphics_progressive_read_tile_data(&stream, parsed.cb_raw_len, &parsed.cb_raw_data) !=
            LIBRDP_STATUS_OK ||
        rdp_graphics_progressive_read_tile_data(&stream, parsed.cr_srl_len, &parsed.cr_srl_data) !=
            LIBRDP_STATUS_OK ||
        rdp_graphics_progressive_read_tile_data(&stream, parsed.cr_raw_len, &parsed.cr_raw_data) !=
            LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *tile = parsed;
    return LIBRDP_STATUS_OK;
