 ...
 */
/*
 * Module: extended input channel serialization and parsing support.
...
{
    rdp_input_channel_header parsed;
    rdp_stream stream;
    ...
    if (rdp_stream_read_u16_le(&stream, &parsed.event_id) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.pdu_length) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (parsed.pdu_length != length)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (!rdp_input_channel_valid_event_id(parsed.event_id))
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *header = parsed;
    return LIBRDP_STATUS_OK;
}
...
{
    rdp_input_channel_sc_ready parsed;
    rdp_stream stream;
    ...
    if (rdp_stream_skip(&stream, 6) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.protocol_version) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (!rdp_input_channel_valid_protocol(parsed.protocol_version))
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (rdp_stream_remaining(&stream) == 4u)
    {
        if (rdp_stream_read_u32_le(&stream, &parsed.supported_features) != LIBRDP_STATUS_OK)
            return LIBRDP_STATUS_PROTOCOL_ERROR;
        if ((parsed.supported_features & ~RDP_INPUT_CHANNEL_SC_READY_MULTIPEN) != 0)
            return LIBRDP_STATUS_PROTOCOL_ERROR;
        parsed.has_supported_features = 1;
    }
    if (parsed.protocol_version == RDP_INPUT_CHANNEL_PROTOCOL_V300 && !parsed.has_supported_features)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *ready = parsed;
    return LIBRDP_STATUS_OK;
}
...
{
    rdp_input_channel_cs_ready parsed;
    rdp_stream stream;
    rdp_input_channel_header header;
    ...
    if (rdp_stream_skip(&stream, 6) != LIBRDP_STATUS_OK ||
        ...
        rdp_stream_read_u32_le(&stream, &parsed.protocol_version) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.max_touch_contacts) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (!rdp_input_channel_valid_protocol(parsed.protocol_version) ||
        (parsed.flags & ~(RDP_INPUT_CHANNEL_CS_SHOW_TOUCH_VISUALS |
                          RDP_INPUT_CHANNEL_CS_DISABLE_TIMESTAMP_INJECTION |
                          RDP_INPUT_CHANNEL_CS_ENABLE_MULTIPEN)) != 0)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (parsed.protocol_version == RDP_INPUT_CHANNEL_PROTOCOL_V100 &&
        (parsed.flags & RDP_INPUT_CHANNEL_CS_DISABLE_TIMESTAMP_INJECTION) != 0)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (parsed.protocol_version < RDP_INPUT_CHANNEL_PROTOCOL_V200 &&
        (parsed.flags & RDP_INPUT_CHANNEL_CS_ENABLE_MULTIPEN) != 0)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *ready = parsed;
    return LIBRDP_STATUS_OK;
}
...
librdp_status rdp_input_channel_parse_dismiss_hovering(const void* data, size_t length, uint8_t* contact_id)
{
    ...
    rdp_stream_init(&stream, data, length);
    if (rdp_stream_skip(&stream, 6) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u8(&stream, &parsed_contact_id) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
...
{
    rdp_input_channel_touch_contact parsed;

    if (!contact)
        return LIBRDP_STATUS_INVALID_ARGUMENT;
    memset(&parsed, 0, sizeof(parsed));
    if (rdp_stream_read_u8(stream, &parsed.contact_id) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(stream, &parsed.fields_present) != LIBRDP_STATUS_OK ||
        rdp_input_channel_read_i32_le(stream, &parsed.x) != LIBRDP_STATUS_OK ||
        rdp_input_channel_read_i32_le(stream, &parsed.y) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(stream, &parsed.contact_flags) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (rdp_input_channel_validate_touch_contact(&parsed) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if ((parsed.fields_present & RDP_INPUT_CHANNEL_TOUCH_CONTACTRECT_PRESENT) != 0)
    {
        if (rdp_input_channel_read_i16_le(stream, &parsed.contact_rect_left) != LIBRDP_STATUS_OK ||
            rdp_input_channel_read_i16_le(stream, &parsed.contact_rect_top) != LIBRDP_STATUS_OK ||
            rdp_input_channel_read_i16_le(stream, &parsed.contact_rect_right) != LIBRDP_STATUS_OK ||
            rdp_input_channel_read_i16_le(stream, &parsed.contact_rect_bottom) != LIBRDP_STATUS_OK)
            return LIBRDP_STATUS_PROTOCOL_ERROR;
    }
    if ((parsed.fields_present & RDP_INPUT_CHANNEL_TOUCH_ORIENTATION_PRESENT) != 0 &&
        rdp_stream_read_u32_le(stream, &parsed.orientation) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if ((parsed.fields_present & RDP_INPUT_CHANNEL_TOUCH_PRESSURE_PRESENT) != 0 &&
        rdp_stream_read_u32_le(stream, &parsed.pressure) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (rdp_input_channel_validate_touch_contact(&parsed) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *contact = parsed;
...
{
    rdp_input_channel_touch_event parsed;
    rdp_stream stream;
    rdp_input_channel_header header;
    ...
    rdp_stream_init(&stream, data, length);
    if (rdp_stream_skip(&stream, 6) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(&stream, &parsed.encode_time) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(&stream, &parsed.frame_count) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (parsed.frame_count == 0)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    parsed.frames_len = rdp_stream_remaining(&stream);
    if (rdp_stream_read_bytes(&stream, &parsed.frames, parsed.frames_len) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (rdp_input_channel_validate_touch_frames(parsed.frames,
                                                parsed.frames_len,
                                                parsed.frame_count) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *event = parsed;
    return LIBRDP_STATUS_OK;
}
...
{
    rdp_input_channel_touch_frame parsed;
    rdp_stream stream;
    uint16_t i = 0;
    ...
    for (i = 0; i <= index; i++)
    {
        ...
        if (i == index)
        {
            parsed.contact_count = contact_count;
            parsed.frame_offset = frame_offset;
            parsed.contacts = contacts;
            parsed.contacts_len = contacts_len;
            *frame = parsed;
            return LIBRDP_STATUS_OK;
        }
    }
...
{
    rdp_input_channel_pen_contact parsed;

    if (!contact)
        return LIBRDP_STATUS_INVALID_ARGUMENT;
    memset(&parsed, 0, sizeof(parsed));
    if (rdp_stream_read_u8(stream, &parsed.device_id) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u16_le(stream, &parsed.fields_present) != LIBRDP_STATUS_OK ||
        rdp_input_channel_read_i32_le(stream, &parsed.x) != LIBRDP_STATUS_OK ||
        rdp_input_channel_read_i32_le(stream, &parsed.y) != LIBRDP_STATUS_OK ||
        rdp_stream_read_u32_le(stream, &parsed.contact_flags) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (rdp_input_channel_validate_pen_contact(&parsed) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if ((parsed.fields_present & RDP_INPUT_CHANNEL_PEN_FLAGS_PRESENT) != 0 &&
        rdp_stream_read_u32_le(stream, &parsed.pen_flags) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if ((parsed.fields_present & RDP_INPUT_CHANNEL_PEN_PRESSURE_PRESENT) != 0 &&
        rdp_stream_read_u32_le(stream, &parsed.pressure) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if ((parsed.fields_present & RDP_INPUT_CHANNEL_PEN_ROTATION_PRESENT) != 0 &&
        rdp_stream_read_u16_le(stream, &parsed.rotation) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if ((parsed.fields_present & RDP_INPUT_CHANNEL_PEN_TILTX_PRESENT) != 0 &&
        rdp_input_channel_read_i16_le(stream, &parsed.tilt_x) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if ((parsed.fields_present & RDP_INPUT_CHANNEL_PEN_TILTY_PRESENT) != 0 &&
        rdp_input_channel_read_i16_le(stream, &parsed.tilt_y) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (rdp_input_channel_validate_pen_contact(&parsed) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *contact = parsed;
...
{
    rdp_input_channel_pen_event parsed;
    rdp_stream stream;
    rdp_input_channel_header header;
    ...
    if (rdp_stream_read_bytes(&stream, &parsed.frames, parsed.frames_len) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    if (rdp_input_channel_validate_pen_frames(parsed.frames,
                                              parsed.frames_len,
                                              parsed.frame_count) != LIBRDP_STATUS_OK)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    *event = parsed;
...
{
    rdp_input_channel_pen_frame parsed;
    rdp_stream stream;
    uint16_t i = 0;
