                                                                         ...
                                                                         uint32_t mode)
{
    ...
    if ((security_information & RDP_FILESYSTEM_REDIRECTION_SACL_SECURITY_INFORMATION) != 0)
    {
        size_t sacl_offset = buffer->length - base;

        control |= 0x0010u;
        rdp_filesystem_security_write_u32_le(buffer->data, base + 12u, (uint32_t)sacl_offset);
        status = rdp_filesystem_append_zeroes(buffer, 8u);
        if (status != LIBRDP_STATUS_OK)
            return status;
        buffer->data[base + sacl_offset] = 2;
        rdp_filesystem_security_write_u16_le(buffer->data, base + sacl_offset + 2u, 8u);
        rdp_filesystem_security_write_u16_le(buffer->data, base + sacl_offset + 4u, 0u);
    }
    ...
    rdp_filesystem_redirection_posix_security* security)
{
    ...
    sacl_offset = rdp_filesystem_security_read_u32_le(bytes, 12u);
    ...
    if ((security_information & RDP_FILESYSTEM_REDIRECTION_SACL_SECURITY_INFORMATION) != 0)
    {
        if ((control & 0x0010u) != 0 && sacl_offset != 0)
        {
            rdp_filesystem_redirection_posix_security ignored;

            memset(&ignored, 0, sizeof(ignored));
            status = rdp_filesystem_security_parse_dacl(bytes, length, sacl_offset, &ignored);
            if (status != LIBRDP_STATUS_OK)
                return status;
        }
    }
...
librdp_status rdp_filesystem_redirection_write_write_response(rdp_buffer* buffer,
                                                              ...
                                                              uint32_t written)
{
    librdp_status status = rdp_filesystem_redirection_write_length_response(
        buffer, device_id, completion_id, io_status, written);

    if (status != LIBRDP_STATUS_OK)
        return status;
    return rdp_buffer_append_u8(buffer, 0u);
}
    ...
    rdp_filesystem_redirection_length_response* response)
{
    ...
    if (length != 21u)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    memset(response, 0, sizeof(*response));
    if (rdp_device_redirection_parse_io_completion(
            data, length, &response->io) != LIBRDP_STATUS_OK ||
        response->io.payload_len != 5u)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    rdp_stream_init(&stream, response->io.payload, response->io.payload_len);
    if (rdp_stream_read_u32_le(&stream, &response->length) !=
            LIBRDP_STATUS_OK ||
        rdp_stream_read_u8(&stream, &padding) != LIBRDP_STATUS_OK ||
        padding != 0u || rdp_stream_remaining(&stream) != 0u)
        return LIBRDP_STATUS_PROTOCOL_ERROR;
    return LIBRDP_STATUS_OK;
}
...
librdp_status rdp_filesystem_redirection_write_lock_response(rdp_buffer* buffer,
                                                             ...
                                                             uint32_t io_status)
{
    static const uint8_t output_length[4] = {0, 0, 0, 0};
    librdp_status status = LIBRDP_STATUS_OK;
    ...
    if (status != LIBRDP_STATUS_OK)
        return status;
    return rdp_buffer_append(buffer, output_length, sizeof(output_length));
}
