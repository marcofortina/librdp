...
#define LIBRDP_MULTITRANSPORT_PROVIDER_MAX_TIMEOUT_MS 600000u /**< Maximum asynchronous bootstrap timeout. */
 ...
 *
 * The values match the protocol identifiers carried by the Message Channel
 * bootstrap request. They identify transport semantics, not a native socket
 * type or a particular TLS implementation.
 *
 * @since 0.1.0
 */
typedef enum librdp_multitransport_protocol
{
    LIBRDP_MULTITRANSPORT_UDP_RELIABLE = 1, /**< Reliable RDPEUDP side transport. */
    LIBRDP_MULTITRANSPORT_UDP_LOSSY = 2     /**< Loss-tolerant RDPEUDP side transport. */
} librdp_multitransport_protocol;

/**
 * @brief Versioned request passed to a side-transport provider.
 *
 * request_token is generated by the session and remains unique across
 * reconnects for the lifetime of that session. The application uses it to
 * complete an asynchronous request and to identify resources released by the
 * provider callback. security_cookie binds the application-created secure
 * side transport to this RDP connection.
 *
 * @since 0.1.0
 */
typedef struct librdp_multitransport_request
{
    uint32_t version; /**< Struct version, LIBRDP_MULTITRANSPORT_REQUEST_VERSION. */
    uint32_t size;    /**< Size of this struct in bytes. */
    uint64_t request_token; /**< Session-local opaque token used for completion and release. */
    uint32_t request_id; /**< Wire request identifier echoed in the Message Channel response. */
    librdp_multitransport_protocol protocol; /**< Requested reliable or lossy transport semantics. */
    uint8_t security_cookie[LIBRDP_MULTITRANSPORT_COOKIE_SIZE]; /**< Connection-binding cookie. */
} librdp_multitransport_request;

/**
 * @brief Begin creation of an application-owned secure side transport.
 *
 * The callback runs synchronously on the serialized session-driving thread.
 * request is borrowed and valid only until the callback returns. Return
 * LIBRDP_STATUS_OK only after the socket or tunnel is connected, protected,
 * and bound with request->security_cookie. Return LIBRDP_STATUS_AGAIN after
 * retaining request_token when completion will occur asynchronously. Any
 * other status rejects the side transport while preserving the primary TCP
 * session.
 *
 * @param[in,out] session Session receiving the bootstrap request; never NULL.
 * @param[in] request Borrowed complete request descriptor; never NULL.
 * @param[in,out] user_data Opaque provider context; may be NULL.
 *
 * @return LIBRDP_STATUS_OK for synchronous success, LIBRDP_STATUS_AGAIN for an
 * accepted asynchronous request, or another status to reject the request.
 *
 * @note Thread-safety: the callback executes on the session owner thread. An
 * asynchronous worker must marshal completion back to that thread before
 * calling librdp_session_complete_multitransport_request().
 * @note Do not re-enter session lifecycle, dispatch, or provider-management
 * APIs from this callback. Return first, then schedule owner-thread work.
 * @warning The security cookie is authentication material. Do not log or
 * persist it, and erase application-owned copies after the request ends.
 * @since 0.1.0
 */
typedef librdp_status (*librdp_multitransport_start_callback)(
    librdp_session* session,
    const librdp_multitransport_request* request,
    void* user_data);

/**
 * @brief Release a pending or established application-owned side transport.
 *
 * The callback runs when a pending request is rejected, times out, is
 * cancelled by teardown, or when an established transport becomes stale at
 * disconnect/reconnect. request is borrowed for the duration of the callback.
 * established is non-zero only when the provider previously completed the
 * request successfully.
 *
 * @param[in,out] session Session releasing the request; never NULL.
 * @param[in] request Borrowed request descriptor; never NULL.
 * @param[in] established Non-zero for an established transport, zero for a
 * pending transport that must be cancelled.
 * @param[in,out] user_data Opaque provider context; may be NULL.
 *
 * @note Thread-safety: the callback executes on the serialized session owner
 * thread. It must not block waiting for an unbounded platform operation.
 * @warning Erase retained security-cookie material before returning.
 * @since 0.1.0
 */
typedef void (*librdp_multitransport_release_callback)(
    librdp_session* session,
    const librdp_multitransport_request* request,
    int established,
    void* user_data);
...
/**
 * @brief Versioned application provider for multitransport bootstrap.
 *
 * The session copies this descriptor. Function pointers and user_data remain
 * borrowed and must stay valid until the provider is removed or the session is
 * freed. Both start and release are mandatory so every pending or established
 * resource has a deterministic teardown path. timeout_ms bounds requests for
 * which start returns LIBRDP_STATUS_AGAIN. Application event loops poll the
 * provider-owned descriptors separately and pass received records to the
 * public multitransport datagram APIs.
 *
 * @since 0.1.0
 */
typedef struct librdp_multitransport_provider
{
    uint32_t version; /**< Struct version, LIBRDP_MULTITRANSPORT_PROVIDER_VERSION. */
    uint32_t size;    /**< Size of this struct in bytes. */
    librdp_multitransport_start_callback start; /**< Mandatory bootstrap callback. */
    librdp_multitransport_release_callback release; /**< Mandatory cancel/teardown callback. */
    void* user_data; /**< Borrowed opaque callback context; may be NULL. */
    uint32_t timeout_ms; /**< Async timeout in milliseconds, from 1 through the public maximum. */
} librdp_multitransport_provider;
 ...
 *
 * The descriptor is cleared, versioned, sized, and assigned a bounded default
 * timeout. The caller must set start and release before installing it on a
 * session.
 *
 * @param[out] provider Caller-owned descriptor; must not be NULL.
 *
 * @return LIBRDP_STATUS_OK on success or LIBRDP_STATUS_INVALID_ARGUMENT when
 * provider is NULL.
 *
 ...
 * @since 0.1.0
 */
LIBRDP_API librdp_status librdp_multitransport_provider_init(
...
/**
 * @brief Install or remove the application side-transport provider.
 *
 * The provider descriptor is copied, while its callbacks and user_data remain
 * borrowed. Passing NULL removes the current provider. A provider cannot be
 * replaced while a side transport is pending or established; disconnect the
 * session first so the release callback can run.
 *
 * @param[in,out] session Session to configure; must not be NULL.
 * @param[in] provider Initialized provider to copy, or NULL to remove it.
 *
 * @return LIBRDP_STATUS_OK on success; LIBRDP_STATUS_INVALID_ARGUMENT for an
 * invalid version, size, callback, or timeout; LIBRDP_STATUS_STATE when called
 * from a non-owner thread or while a side transport is pending/established.
 *
 * @note Thread-safety: call from the serialized session owner thread. The
 * provider may be installed before the first connect call.
 * @warning Keep provider callbacks and user_data alive until removal or
 * session destruction. The core never logs provider security cookies.
 * @since 0.1.0
 */
LIBRDP_API librdp_status librdp_session_set_multitransport_provider(
    librdp_session* session,
    const librdp_multitransport_provider* provider);

/**
 * @brief Complete an asynchronous side-transport bootstrap request.
 *
 * Call this after a provider start callback returned LIBRDP_STATUS_AGAIN.
 * result must be LIBRDP_STATUS_OK when the side transport is connected,
 * protected, and security-cookie bound. Any other terminal status rejects the
 * side transport and leaves TCP authoritative. The opaque request token makes
 * completions from an earlier reconnect epoch fail safely.
 *
 * @param[in,out] session Session that issued request_token; must not be NULL.
 * @param[in] request_token Non-zero token from the provider request.
 * @param[in] result LIBRDP_STATUS_OK or a terminal failure status; must not be
 * LIBRDP_STATUS_AGAIN.
 *
 * @return LIBRDP_STATUS_OK after the correlated response is sent;
 * LIBRDP_STATUS_INVALID_ARGUMENT for a zero token or AGAIN result;
 * LIBRDP_STATUS_STATE for a stale, unknown, already completed, or cross-thread
 * request; transport and allocation errors from the Message Channel response.
 *
 * @note Thread-safety: marshal completion onto the serialized session owner
 * thread. Worker threads may signal the application event loop but must not
 * call this function directly.
 * @warning A successful result asserts that the application has validated and
 * protected the side transport; returning success prematurely weakens the
 * connection binding.
 * @since 0.1.0
 */
LIBRDP_API librdp_status librdp_session_complete_multitransport_request(
    librdp_session* session,
